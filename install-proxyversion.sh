...
if [[ $DISTRO == *"ubuntu"* ]] || [[ $DISTRO == *"Ubuntu"* ]]; then
    ...
    echo "UV_THREADPOOL_SIZE=$THREAD screen -dmS NIMIQ-MINER node index.js --host $DOMAIN --port 8080 -key /etc/ssl/ssl/private.key --cert /etc/ssl/ssl/public.crt --miner=$THREAD --wallet-seed=$SEED" >> start-miner.sh
